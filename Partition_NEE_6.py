...
import light_and_T_response_functions_version3 as light
...
def main(data_dict, configs_dict):
    ...
    if configs_dict['output_plots'] | configs_dict['output_results']:
    ...
    param_rslt_list = ['Eo', 'rb', 'alpha', 'beta', 'k']
...
def estimate_Re(sub_d, params_d):
    ...
    GPP, Re = light.LRF(sub_d, params_d['Eo'], params_d['rb'],
                        params_d['alpha'], params_d['beta'], params_d['k'])
...
def make_initial_guess_dict(data_d):
    ...
    index = np.where(data_d['PAR'] > 10)[0]
    d['beta_prior'] = (np.nanpercentile(data_d['NEE'][index], 5) - 
